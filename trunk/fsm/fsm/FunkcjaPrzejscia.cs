		...
		public class FunkcjaPrzejscia {
				...
				public void DodajStan(string nazwa, bool koncowy) {
						foreach (Stan e in Stany) if (e.nazwa == nazwa)
										throw new ExceptionInFunkcjaPrzejscia(Language.lang[45]);
				...
				public void DodajPrzejscie(string nazwaStanu, char litera, string stanDocelowy) {
						...
						if (start == null) throw new ExceptionInFunkcjaPrzejscia(Language.lang[46] + nazwaStanu);
						Stan koniec = null;
						foreach (Stan i in Stany) if (i.nazwa == stanDocelowy) koniec = i;
						if (koniec == null) throw new ExceptionInFunkcjaPrzejscia(Language.lang[47] + stanDocelowy);
						bool lit = true;
						foreach (char i in alfabet) if (i == litera) lit = false;
						if (lit) throw new ExceptionInFunkcjaPrzejscia(Language.lang[48] + litera);
						char[] c = start.mapa.Keys.ToArray();
						lit = false;
						foreach (char i in c) if (i == litera) lit = true;
						if (lit) throw new ExceptionInFunkcjaPrzejscia(Language.lang[49]);
				...
				public void Przejscie(char litera) {
						...
						if (s != null) obecny = s;
						else throw new ExceptionInFunkcjaPrzejscia(Language.lang[50]);
